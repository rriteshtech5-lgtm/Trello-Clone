...
export default function DashboardPage() {
  const { user } = useUser();
  const { createBoard, deleteBoard, boards, error } = useBoards();
  ...
  const handleDeleteBoard = async (e: React.MouseEvent, boardId: string) => {
    ...
    const ok = window.confirm("Delete this board? This action cannot be undone.");
    if (!ok) return;
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
              {filteredBoards.map((board, key) => (
                <div key={key} className="relative">
                  <button
                    className="absolute top-3 right-3 z-10 p-1 rounded hover:bg-red-100 text-red-500"
